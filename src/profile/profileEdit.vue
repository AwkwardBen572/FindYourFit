...
const registerUserInfo = async () => {
    ...
    const user = {
        ...
        uid: userData.uid ? userData.uid : currentUser.uid,
        loginDate: userData.loginDate,
        streak: {
            count: userData.streak.count,
            lastUpdate: userData.streak.lastUpdate
        },
    ...
    await setDoc(doc(db, "users", userData.uid ? userData.uid : currentUser.uid), user)
